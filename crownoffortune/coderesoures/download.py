...
BASE='https://casino-wbgame.jiligames.com/cof/'
...
def batch(paths):
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as pool:return list(pool.map(get,sorted(set(paths))))
def main():
    paths=['index.html','index.a7388.js','application.6c543.js','src/system.bundle.692e9.js','src/import-map.e7209.json','src/settings.fbe70.json','cocos-js/cc.af2c4.js','src/chunks/bundle.efe59.js','src/zip-bundler/fflate.min.fdb1f.js']
    batch(paths)
    settings=json.loads((ROOT/'src/settings.fbe70.json').read_bytes());configs={};imports=[]
    for name,ver in settings['assets']['bundleVers'].items():
        ...
        extensions={int(i) if str(i).isdigit() else i:ext for ext,ids in c.get('extensionMap',{}).items() for i in ids}
        v=c['versions']['import']
    ...
    for name,c in configs.items():
        image_ext={};native_names={}
        import sys
        sys.path.insert(0,str(ROOT.parent/'tools'))
        from recover import decode
        for packid,ids in c['packs'].items():
            ...
            if isinstance(d,dict):
                if d.get('type')=='cc.ImageAsset':
                    for idx,entry in zip(ids,d['data']):
                        first=entry[5][0]
            ...
            for idx,e in zip(ids,d[5]):
                ...
                if isinstance(first,list):
                    obj=decode(d,e)[0]
                    if obj.get('_native'):
                        image_ext[idx]=obj['_native'] if obj['_native'].startswith('.') else Path(obj['_native']).suffix
                        if not obj['_native'].startswith('.'):native_names[idx]=obj['_native']
            ...
            if ext is None:
                for p in files:
                    if p and p.suffix=='.json' and p.name.startswith(u+'.'):
                        ...
                        if m:ext=m[1]
            suffix='/'+native_names[idx] if idx in native_names else ext or '.png'
            native.append(f'assets/{name}/native/{u[:2]}/{u}.{ver}{suffix}')
    batch(native)
