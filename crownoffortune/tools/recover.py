...
def main():
    ...
    for bundle in SOURCE.iterdir():
        ...
        for packid,ids in cfg['packs'].items():
            ids=[int(i) if str(i).isdigit() else i for i in ids]
            ...
            for i,entry in enumerate(data[5]):
                first=entry[0][0]; uid=expand(cfg['uuids'][ids[i]] if isinstance(ids[i],int) else ids[i])
                if isinstance(first,dict) and 'rect' in first and 'name' in first:
                    ...
                    imageuid=str(uuid.uuid5(uuid.NAMESPACE_URL,'crownoffortune/sprite/'+uid))
                    ...
                    if typ=='sp.SkeletonData':
                        obj=decode(data,entry)[0];folder=ASSETS/'spine'/(obj['_name']+'_'+uid[:8])
                        folder.mkdir(parents=True,exist_ok=True)
                        if obj.get('_skeletonJson'):
                            path=folder/(obj['_name']+'.json');save(path,obj['_skeletonJson'])
                        else:
                            path=folder/(obj['_name']+'.skel');path.write_bytes(next((bundle/'native').rglob(uid+'.*.bin')).read_bytes())
