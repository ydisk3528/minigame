...
export class MultiScrollView extends Component {
    ...
    show() {
        ...
        if (index === 0) {
            this.currentPosition = new Vec3(0, this.pve1Node.position.y + this.pve1Prefab.data.getChildByName('level1').y, 0);
        } else {
            let idxPage = Math.floor((index - 1) / 22);
            let idxLevel = (index - 1) % 22 + 1;
            this.currentPosition = new Vec3(0, this.positions[idxPage].y + this.pve2Prefab.data.getChildByName('level' + idxLevel).y, 0);
        }
    ...
    scrollToNode() {
        if (this.isNeedScroll) {
            const maxOffsetY = this.scrollView.getMaxScrollOffset().y;
            this.scrollView.scrollTo(new Vec2(
                0, maxOffsetY > 0 ? Math.min(1, Math.max(0, this.currentPosition.y / maxOffsetY)) : 0), 0.1);
        }
    }
