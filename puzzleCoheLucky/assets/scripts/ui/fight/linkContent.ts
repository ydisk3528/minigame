import { _decorator, Component, Prefab, Node, UITransform, Vec3, instantiate, Animation, Graphics, Color } from 'cc';
...
export class LinkContent extends Component {
    ...
    show(fightScene: any) {
        ...
        this.boardMask = this.parseBoardMask(this.levelInfo.boardMask);
        this.drawBoardGrid();
        this.initCake();
    }

    parseBoardMask(value: any) {
        const raw = String(value || '').replace(/[^01]/g, '');
        const mask = raw.length === constants.LINK_ROWS_COUNT * constants.LINK_COLS_COUNT
            ? raw.split('').map((cell) => cell === '1')
            : new Array(constants.LINK_ROWS_COUNT * constants.LINK_COLS_COUNT).fill(true);
        return this.findPlayableChain(mask).length === 3
            ? mask
            : new Array(constants.LINK_ROWS_COUNT * constants.LINK_COLS_COUNT).fill(true);
    }

    findPlayableChain(mask = this.boardMask) {
        for (let index = 0; index < mask.length; index++) {
            if (!mask[index]) continue;
            const pos = this.getPosByIndex(index);
            const neighbors = [];
            for (let row = pos.y - 1; row <= pos.y + 1; row++) {
                for (let col = pos.x - 1; col <= pos.x + 1; col++) {
                    if (row === pos.y && col === pos.x) continue;
                    if (row < 0 || col < 0 || row >= constants.LINK_ROWS_COUNT || col >= constants.LINK_COLS_COUNT) continue;
                    const neighbor = this.getIndexByPos(col, row);
                    if (mask[neighbor]) neighbors.push(neighbor);
                }
            }
            if (neighbors.length >= 2) return [index, neighbors[0], neighbors[1]];
        }
        return [];
    }
    ...
    isCellActive(index: number) {
        return this.boardMask[index] === true;
    }

    drawBoardGrid() {
        if (!this.gridNode) {
            this.gridNode = new Node('boardGrid');
            this.gridNode.layer = this.node.layer;
            this.node.addChild(this.gridNode);
            this.gridNode.setSiblingIndex(0);
            const transform = this.gridNode.addComponent(UITransform);
            transform.setAnchorPoint(0, 0);
            transform.setContentSize(this.node.getComponent(UITransform)!.contentSize);
        }
        const graphics = this.gridNode.getComponent(Graphics) || this.gridNode.addComponent(Graphics);
        graphics.clear();
        for (let index = 0; index < this.boardMask.length; index++) {
            if (!this.isCellActive(index)) continue;
            const pos = this.getPosByIndex(index);
            const x = pos.x * CELL_WIDTH + 8;
            const y = pos.y * CELL_HEIGHT + 8;
            const width = CELL_WIDTH - 16;
            const height = CELL_HEIGHT - 16;
            graphics.fillColor = new Color(35, 25, 92, 95);
            graphics.roundRect(x + 2, y - 3, width, height, 13);
            graphics.fill();
            graphics.fillColor = new Color(133, 111, 221, 175);
            graphics.strokeColor = new Color(236, 222, 255, 205);
            graphics.lineWidth = 2;
            graphics.roundRect(x, y, width, height, 13);
            graphics.fill();
            graphics.stroke();
            graphics.fillColor = new Color(255, 255, 255, 35);
            graphics.roundRect(x + 4, y + height - 11, width - 8, 7, 4);
            graphics.fill();
        }
    }
    ...
    fillEmptyCell() {
        for (let idxCol = 0; idxCol < constants.LINK_COLS_COUNT; idxCol++) {
            ...
            for (let idxRow = 0; idxRow < constants.LINK_ROWS_COUNT; idxRow++) {
                ...
                if (this.isCellActive(index)) activeIndices.push(index);
            }
            const nodes = activeIndices
                .filter((index) => this.dictCakes.hasOwnProperty(index))
                .map((index) => this.dictCakes[index]);
            activeIndices.forEach((index) => delete this.dictCakes[index]);
            for (let idx = 0; idx < nodes.length; idx++) {
                const targetIndex = activeIndices[idx];
                const linkItem = nodes[idx].getComponent(LinkItem);
                this.dictCakes[targetIndex] = nodes[idx];
                linkItem.index = targetIndex;
                linkItem.playMove2Index(targetIndex);
            }
            const arrRandom = this.getLevelCakes();
            for (let idx = nodes.length; idx < activeIndices.length; idx++) {
                const showIndex = (constants.LINK_ROWS_COUNT + idx - nodes.length) * constants.LINK_COLS_COUNT + idxCol;
                const randomCake = arrRandom[loadsh.random(0, arrRandom.length - 1)];
                this.createCakeForFill(activeIndices[idx], randomCake, showIndex);
            }
        }
        this.scheduleOnce(() => {
            this.checkIsBlocked();
