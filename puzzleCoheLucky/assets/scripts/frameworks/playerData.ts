 ...
 */
    ...
    */
    ...
    finishLevel(level: any, score: any) {
        ...
        let propReward = null;
        if (preStar === 0 && star > 0 && levelInfo.propReward && levelInfo.propReward !== 'none') {
            const reward = String(levelInfo.propReward).split(':');
            const propId = reward[0] === 'random'
                ? [constants.PROP_ID.HAMMER, constants.PROP_ID.MAGIC, constants.PROP_ID.REFRESH][Math.floor(Math.random() * 3)]
                : Number(reward[0]);
            const amount = Math.max(1, Number(reward[1]) || 1);
            if (propId >= constants.PROP_ID.HAMMER && propId <= constants.PROP_ID.INFINITE) {
                this.addProp(propId, amount);
                clientEvent.dispatchEvent('updateProp', propId);
                propReward = { propId, amount };
            }
        }

        return { levelId: level, score: score, star: star, gold: gold, propReward, costTime: costTime, spareStep: this.spareStep };
    }
