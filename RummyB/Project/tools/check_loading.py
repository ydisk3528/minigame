...
def main():
    scene=ASSETS/'scenes/RummyB.scene'
    data=json.loads(scene.read_bytes())
    names={o.get('_name') for o in data if o.get('__type__')=='cc.Node'}
    assert not names.intersection({'TableView','ResultView','Help','ChangeAvatorView','RoomMenu'})
    first=dependencies(scene)
    assert not any(p.suffix=='.mp3' or 'resources/deferred' in p.as_posix() for p in first)
    full=dependencies(ASSETS/'prefabs/OfflineApp.prefab')
    native=lambda paths:{p for p in paths if p.is_file() and p.suffix in {'.png','.mp3'}}
    first_native=native(first);full_native=native(full)
    table=native(dependencies(ASSETS/'resources/deferred/TableView.prefab'))
    report={
