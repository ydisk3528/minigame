...
function conserved(round){const cards=[...round.stock,...round.pile,round.indicator,...round.seats.flatMap(s=>s.hand)];assert.equal(cards.length,108);assert.equal(new Set(cards.map(c=>c.id)).size,108);}
...
// Auto Sort reports whether visible order/grouping changes, including an already sorted hand.
const sorted=new Round(1,()=>.43);
assert.equal(sorted.sort(),false,'already sorted hand has no changes');
const group=sorted.seats[0].groups.find(g=>g.length>1);
assert.ok(group);group.reverse();
assert.equal(sorted.sort(),true,'manual disorder can be sorted');
assert.equal(sorted.sort(),false,'sorting again has no changes');
console.log('PASS: Auto Sort changed/unchanged result');
