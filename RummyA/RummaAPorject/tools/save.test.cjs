...
let s=loadSave(storage);assert.equal(s.balance,0);assert.equal(s.music,true);assert.equal(s.sound,true);
map.set('rummy.music','off');map.set('rummy.sound','off');
assert.equal(loadSave(storage).music,false);assert.equal(loadSave(storage).sound,false);
map.delete('rummy.music');map.delete('rummy.sound');
s.music=false;s.sound=false;
let native='';const bridge={getGameSave:()=>native,setGameSave:v=>native=v};
storeSave(s,storage,bridge);assert.equal(native,map.get('rummy.save.v1'));assert.equal(loadSave(storage,bridge).music,false);assert.equal(loadSave(storage,bridge).sound,false);
