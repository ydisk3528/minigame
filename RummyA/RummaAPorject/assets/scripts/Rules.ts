...
export function deck(){let out=[];for(let copy=0;copy<2;copy++){for(let suit=0;suit<4;suit++)for(let rank=1;rank<=13;rank++)out.push({id:out.length,suit,rank});out.push({id:out.length,suit:4,rank:0},{id:out.length+1,suit:4,rank:0});}return out;}
