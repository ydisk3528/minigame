...
SOURCE = PROJECT / 'coderesoures/assets'
ASSETS = PROJECT / 'assets'
ABC = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
REPORT = {'sourceEngine': '3.6.2', 'sprites': [], 'prefabs': [], 'animations': [], 'omittedComponents': {}, 'missingReferences': []}
...
def main():
    mapping={}; prefabs={}; scenes={}; images={p.name.split('.')[0]:p for p in SOURCE.rglob('*') if p.suffix in ('.png','.webp','.jpg')}
    packs=[]
    for bundle in SOURCE.iterdir():
        configs=list(bundle.glob('config.*.json'))
        if not configs: continue
        cfg=json.loads(configs[0].read_bytes())
        jobs=[];packed=set()
        for packid,ids in cfg['packs'].items():
            file=next((bundle/'import').rglob(packid+'.*.json'),None)
            if not file:continue
            packed.add(file);data=json.loads(file.read_bytes())
            if isinstance(data,list):jobs.append((data,ids))
        for file in (bundle/'import').rglob('*.json'):
            if file in packed:continue
            data=json.loads(file.read_bytes())
            if isinstance(data,list) and len(data)==11:jobs.append((data[:5]+[[data[5:]]],[file.name.split('.')[0]]))
        for data,ids in jobs:
            packs.append((bundle,cfg,data,ids))
            for i,entry in enumerate(data[5]):
                first=entry[0][0]; uid=expand(cfg['uuids'][int(ids[i])] if str(ids[i]).isdigit() else ids[i])
                if isinstance(first,dict) and 'rect' in first and 'name' in first:
                    ...
                    if texture not in images:
                        REPORT['missingReferences'].append('Missing atlas '+texture);continue
                    ...
                    imageuid=str(uuid.uuid5(uuid.NAMESPACE_URL,'bc100/sprite/'+uid))
                    ...
                    if typ=='sp.SkeletonData':
                        ...
                        path=folder/(obj['_name']+('.json' if '_skeletonJson' in obj else '.skel'))
                        if '_skeletonJson' in obj:save(path,obj['_skeletonJson'])
                        else:
                            src=next(SOURCE.rglob(uid+'.*.bin'),None)
                            if not src:continue
                            path.parent.mkdir(parents=True,exist_ok=True);path.write_bytes(src.read_bytes())
                        meta(path,uid,'spine-data','1.2.5')
    ...
    for file in SOURCE.rglob('*.cconb'):
        ...
        uid=expand(file.name.split('.')[0]);animations[uid]=unbin(doc);mapping[uid]=uid
    ...
    for uid,objects in prefabs.items():
        def dictionaries(v):
            if isinstance(v,dict):
                yield v
                for x in list(v.values()):yield from dictionaries(x)
            elif isinstance(v,list):
                for x in v:yield from dictionaries(x)
        for obj in list(dictionaries(objects)):
            if isinstance(obj,dict) and obj.get('__type__')=='331d3DyAYFNrLjhRRl6vJvK':
                n=objects[obj['node']['__id__']]
                for ref in n.get('_components',[]):
                    c=objects[ref['__id__']] if '__id__' in ref else ref
                    if c.get('__type__')=='cc.Label':
                        c['_string']=(common if obj.get('useCommon') else lang).get(str(obj.get('key') or obj.get('no')),c.get('_string',''))
        ...
        name=objects[0].get('_name','Prefab');path=ASSETS/'prefabs'/f'{name}_{uid[:8]}.prefab'
