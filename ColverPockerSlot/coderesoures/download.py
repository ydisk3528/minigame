...
BASE='https://casino-wbgame.jiligames.com/bc100/'
...
def main():
    paths=['index.html','index.48727.js','application.e9ff0.js','src/settings.51fc9.json','src/import-map.e7209.json','src/polyfills.bundle.5adbf.js','src/system.bundle.692e9.js']
    batch(paths)
    settings=json.loads((ROOT/'src/settings.51fc9.json').read_bytes());configs={};imports=[]
    for name,ver in settings['assets']['bundleVers'].items():
        ...
        for idx,ver in zip(v[::2],v[1::2]):
            ...
            imports.append(pre+f'import/{u[:2]}/{u}.{ver}'+extensions.get(str(idx),'.json'))
    files=batch(imports);native=[]
    batch(['src/'+x for x in settings.get('plugins',{}).get('jsList',[])]+[x.removeprefix('../') for x in settings.get('scripting',{}).get('scriptPackages',[])])
    for name,c in configs.items():
        ...
        for packid,ids in c['packs'].items():
            ...
            if isinstance(d,dict) and d.get('type') != 'cc.ImageAsset':continue
            entries=[x[5] for x in d['data']] if isinstance(d,dict) else d[5]
            for idx,e in zip(ids,entries):
                first=e[0] if isinstance(d,dict) else e[0][0]
                if isinstance(first,list):
                    mask=d[4][first[0]];cl=d[3][mask[0]]
                    if isinstance(cl,list):
                        for k,vv in enumerate(first[1:],1):
                            if cl[1][mask[k]]=='_native':image_ext[int(idx)]=vv
                if isinstance(first,dict) and 'fmt'in first:
                    formats=[int(x.split('@')[0]) for x in first['fmt'].split('_')]
                    image_ext[int(idx)]=['.png','.jpg','.jpeg','.bmp','.webp','.pvr','.pkm','.astc'][next((x for x in formats if x<5),formats[0])]
        v=c['versions']['native']
        for idx,ver in zip(v[::2],v[1::2]):
            ...
            if typ and c['types'][typ[1]]=='sp.SkeletonData':ext='.bin'
            if ext is None:
                for p in files:
