...
export class MatchEffects {
    ...
    private static readonly COLORS = [
        [1, 0.36, 0.16], [0.18, 0.44, 1], [0.72, 0.84, 0.9],
        [1, 0.8, 0.08], [0.68, 0.2, 1], [1, 0.16, 0.75],
    ] as const;
    ...
    private async playHide(item: ClearEffectItem, index: number): Promise<void> {
        ...
        view.size(128, 128);
        view.pivot(64, 64);
        view.pos((item.column + 0.5) * GameConfig.cellSize, (item.row + 0.5) * GameConfig.cellSize);
        view.alpha = 0.68;
        view.blendMode = "add";
    ...
    private async playBurst(item: ClearEffectItem, useLight: boolean, size: number, quiet = false): Promise<void> {
        ...
        view.pos((item.column + 0.5) * GameConfig.cellSize - view.width / 2,
            (item.row + 0.5) * GameConfig.cellSize - view.height / 2);
        this.effectLayer.addChild(view);
    ...
    private async playBeam(index: number, vertical: boolean): Promise<void> {
        ...
        view.pos(vertical ? (index + 0.5) * GameConfig.cellSize : this.columns * GameConfig.cellSize / 2,
            vertical ? this.rows * GameConfig.cellSize / 2 : (index + 0.5) * GameConfig.cellSize);
        view.scale((vertical ? this.rows : this.columns) * GameConfig.cellSize / view.width, 1);
    ...
    private async playWave(items: ClearEffectItem[]): Promise<void> {
        ...
        view.pos((column + 0.5) * GameConfig.cellSize - view.width / 2,
            (row + 0.5) * GameConfig.cellSize - view.height / 2);
        this.effectLayer.addChild(view);
    ...
    private async playBombOverlay(items: ClearEffectItem[]): Promise<void> {
        ...
        view.pos((center.column + 0.5) * GameConfig.cellSize, (center.row + 0.5) * GameConfig.cellSize);
        this.effectLayer.addChild(view);
    ...
    private async playRainbowOverlay(): Promise<void> {
        ...
        view.pos(this.columns * GameConfig.cellSize / 2, this.rows * GameConfig.cellSize / 2);
        this.effectLayer.addChild(view);
    ...
    private async playComboBanner(cascade: number): Promise<void> {
        ...
        view.pos(this.columns * GameConfig.cellSize / 2, 300);
