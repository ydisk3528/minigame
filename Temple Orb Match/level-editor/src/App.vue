...
const gemNames = ["珊瑚橙", "宝石蓝", "月光银", "琥珀金", "紫罗兰", "玫瑰粉"];
...
const gemColors = ["#ff5c29", "#2e70ff", "#b8d6e6", "#ffcc14", "#ad33ff", "#ff29bf"];
...
function downloadBatchZip() { const levels = batchLevels.value.length ? batchLevels.value : batch(); const errorCount = refreshBatchReport(); if (errorCount) return notice.value = `批量检查失败：${errorCount} 个错误`; const files = levels.map(level => ({ name: filename(level.level), data: JSON.stringify(level, null, 2) })); files.push({ name: "catalog.json", data: JSON.stringify(makeCatalog(levels), null, 2) }); downloadBlob(createZip(files), `temple_orb_levels_${levels[0].level}-${levels.at(-1).level}.zip`); notice.value = `ZIP 已生成：${levels.length} 个关卡和 catalog.json`; }
...
<template>
  <main class="app-shell">
    <header class="hero"><div><p class="eyebrow">TEMPLE ORB MATCH · LEVEL LAB</p><h1>关卡编辑器</h1><p>单关卡和批量规则完全分开，先选择你现在要做的事情。</p></div><div class="status-pill"><i></i>{{ editorMode==='single' ? `单关卡 · ${statusText}` : `批量规则 · ${batchReport.length ? batchReport.length+' 关已检查' : '等待检查'}` }}</div></header>
