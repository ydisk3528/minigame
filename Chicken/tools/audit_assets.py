...
for p in [*assets.glob('scenes/*.scene'),*assets.glob('resources/deferred/*.prefab')]:
    ...
    if p.stem=='ChickenGame':
        road=next(o for o in doc if o.get('__type__')=='cc.Node' and o.get('_name')=='BGLayer')
        assert all(doc[c['__id__']]['__type__']!='cc.Layout' for c in road['_components']),'road positions must not be overridden by Layout'
    for o in doc:
...
scene=json.loads((assets/'scenes/Chicken.scene').read_bytes());assert not any('soundNames'in o for o in scene)
