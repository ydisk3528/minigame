...
exports.Round = exports.HANDS = void 0;
exports.rank = rank;
exports.compare = compare;
exports.shuffle = shuffle;
exports.HANDS = ['HIGH CARD', 'PAIR', 'COLOR', 'SEQUENCE', 'PURE SEQUENCE', 'TRAIL'];
function rank(cards) {
    if (cards.length !== 3 || new Set(cards).size !== 3 || cards.some(c => !Number.isInteger(c) || c < 0 || c > 51))
...
class Round {
    constructor(boot, balances, dealer, random = Math.random) {
        this.boot = boot;
        this.dealer = dealer;
        this.pot = 0;
        this.ended = false;
        ...
        this.award = 0;
        this.pending = null;
        if (!Number.isSafeInteger(boot) || boot <= 0 || balances.length !== 5 || balances.some(n => !Number.isSafeInteger(n) || n < boot * 512))
            throw Error('Invalid room balance');
    ...
    see(i) { if (this.ended || this.players[i].folded || this.pending)
        return; this.players[i].seen = true; this.players[i].last = 'SEEN'; }
    showTarget(i = this.turn) {
        if (this.alive.length === 2)
            return this.alive.find(j => j !== i);
        if (!this.players[i].seen)
            return -1;
        for (let n = 1; n < 5; n++) {
            const j = (i - n + 5) % 5;
            if (!this.players[j].folded)
                return this.players[j].seen ? j : -1;
        }
        return -1;
    }
    can(action, i = this.turn) {
        if (this.ended || this.pending || i !== this.turn || this.players[i].folded)
            return false;
        if (action === 'pack')
            return true;
        if (action === 'show') {
            const target = this.showTarget(i);
            if (target < 0 || (this.alive.length === 2 && this.players[i].seen && !this.players[target].seen))
                return false;
        }
        if (action === 'raise' && this.stake >= this.boot * 128)
            return false;
    ...
    act(action) {
        ...
        if (action === 'pack') {
            ...
            if (action === 'raise')
                this.stake = Math.min(this.stake * 2, this.boot * 128);
            const amount = this.cost(i);
            p.money -= amount;
            p.paid += amount;
            this.pot += amount;
            p.last = action === 'raise' ? 'RAISE' : p.seen ? 'CHAAL' : 'BLIND';
            if (!p.seen && ++p.blinds >= 4)
                this.see(i);
            if (action === 'show') {
                const target = this.showTarget(i);
                if (this.alive.length === 2) {
    ...
    respond(accept) {
        if (!this.pending)
            throw Error('No side show');
        const { from, to } = this.pending;
        this.pending = null;
        if (accept)
            this.duel(from, to);
        else {
            this.players[from].last = 'DECLINED';
            this.advance();
        }
    }
    duel(from, to) {
        ...
        this.players[loser].last = 'LOST SHOW';
        this.advance();
    }
    advance() {
        if (this.alive.length === 1 || this.pot >= this.boot * 1024) {
            this.finish();
            return;
        }
        do
            this.turn = (this.turn + 1) % 5;
        while (this.players[this.turn].folded);
    }
    finish() {
        if (this.ended)
            return;
        ...
        for (const i of this.alive)
            if (compare(this.players[i].cards, this.players[best].cards) > 0)
                best = i;
        this.winners = this.alive.filter(i => compare(this.players[i].cards, this.players[best].cards) === 0);
        this.award = this.pot;
        const each = Math.floor(this.pot / this.winners.length);
        let remainder = this.pot % this.winners.length;
        for (const i of this.winners)
            this.players[i].money += each + (remainder-- > 0 ? 1 : 0);
        this.pot = 0;
        this.ended = true;
    }
}
